...
internal class ViewFilterUtils
{
    ...
    internal static void AddDsfFilter(Document doc, View3D view, string elementMark)
    {
        ...
        var filter = FindFilter(doc, _defaultViewFilterName)
                     ?? GenerateFilter(doc, _defaultViewFilterName, categories,
                         GenerateContainsFilterRule(elementMark));
        ...
        var solidFillPattern = elements.OfClass(typeof(FillPatternElement)).Cast<FillPatternElement>()
    ...
    private static ParameterFilterElement GenerateFilter(Document doc, string name,
        IEnumerable<BuiltInCategory> categories, IEnumerable<FilterRule> rules)
    {
    ...
    {
        return new LogicalAndFilter(filterRules.Select(r => new ElementParameterFilter(r)).Cast<ElementFilter>()
            .ToList());
    }
}